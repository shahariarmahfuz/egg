...
def _do_orm_execute(execute_state):
    if execute_state.is_select:
        if has_request_context() and getattr(g, 'business_id', None):
            # Add criteria with unbound bindparam
            execute_state.statement = execute_state.statement.options(
                with_loader_criteria(
                    db.Model,
                    lambda cls: cls.business_id == bindparam('b_id') if hasattr(cls, 'business_id') and cls.__name__ != 'Business' else True,
                    include_aliases=True,
                    propagate_to_loaders=True
                )
            )
            # Provide the parameter value
            execute_state.statement = execute_state.statement.params(b_id=g.business_id)
