...
def _do_orm_execute(execute_state):
    ...
    if execute_state.is_select:
        def add_tenant_criteria(cls):
            if hasattr(cls, 'business_id') and cls.__name__ != 'Business':
                return cls.business_id == getattr(g, 'business_id', None)
            return True
        ...
        if has_request_context() and getattr(g, 'business_id', None):
            execute_state.statement = execute_state.statement.options(
                with_loader_criteria(
                    db.Model,
                    lambda cls: cls.business_id == g.business_id if hasattr(cls, 'business_id') else None,
